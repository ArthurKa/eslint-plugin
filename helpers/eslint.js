...
const notAdminFolderRE = /^(?!.*packages[\\/]website[\\/]src[\\/]components[\\/]Admin[\\/]).*/;
...
const matchReplacePatterns = [
  ...
  [/import React from 'react';\n/, ['Probably no need to import React?'], '', notAdminFolderRE],
  [/(import )React, (\{)/, ['Probably no need to import React?'], '$1$2', notAdminFolderRE],
...
if(foundSomeErrors) {
  console.info();
  console.error('Some errors were found. Try to run `npm run repo-fix` to fix them.');
  console.info();
